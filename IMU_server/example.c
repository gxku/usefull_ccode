...
void do_kalman_filter(float AccX, float AccY, float AccZ, float MagX, float MagY, float MagZ,
    ...
    if(inited==0){
        ...
        KalmanStep(K, *angles, GyroR);
        KalmanStep(K + 1, *(angles + 1), GyroP);
        KalmanStep(K + 2, *(angles + 2), GyroY);
        printf("%f %f %f \n", K[0].angle, K[1].angle, K[2].angle);
    }
}
