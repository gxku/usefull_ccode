...
def file2_data_show():
    ...
    for line in file:
        q = Quaternion(1,0,0,0)
        q.rotate_euler(float(line.split()[2+3])*0.001,float(line.split()[0+3]),float(line.split()[1+3]))
        cube.draw(screen,q)
        pygame.display.flip()
        pygame.time.delay(50) 
        cube.erase(screen)
...
def process_data_show():
    #printf_data printf("%f %f %f\n",Roll,Pitch,Yaw);
    proc = Popen("../udp_server", bufsize=1024, stdin=PIPE, stdout=PIPE)
